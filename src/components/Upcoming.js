...
class Upcoming extends Component {
    componentDidMount() {
        ...
        axios.get(`https://api.themoviedb.org/3/movie/upcoming?api_key=c0ba1f468f4848a2eb2a4855af9c31d8&region=US`)
        .then(response => {
    ...
    handleScroll = (e) => {
        ...
        if (clientHeight + scrollTop === scrollHeight) {
            axios.get(`https://api.themoviedb.org/3/movie/upcoming?api_key=c0ba1f468f4848a2eb2a4855af9c31d8&region=US&page=${this.props.upcoming.page+1}`)
            .then(response => this.props.startAddNowMovies(response.data.results));
        }    
    }

    render () {
        return (this.props.upcoming.loaded &&
            <div className="Search">
                <div className='movieCardList'>
                    {this.props.upcoming.movies.length > 0 && this.props.upcoming.movies.map(movie => <MovieCard title={movie.title} year={movie.release_date} rating={movie.vote_average} poster={movie.poster_path} key={uuid()} id={movie.id} overview={movie.overview}/>)}
                </div>
                {/* INIZIARE QUI! 
